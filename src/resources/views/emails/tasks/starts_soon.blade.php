...
Se le notifica que tiene una tarea programada para iniciar @if($daysUntilStart === 0) **HOY** @else **MAÑANA** @endif.

<x-mail::panel>
**Proyecto:** {{ $task->projectGroup->project->name ?? 'N/A' }}<br>
**Tarea:** {{ $task->title }}<br>
**Fecha de Inicio:** {{ $task->calculated_start_date ? $task->calculated_start_date->format('d/m/Y') : 'N/A' }}<br>
**Fecha de Entrega:** {{ $task->calculated_end_date ? $task->calculated_end_date->format('d/m/Y') : 'N/A' }}
</x-mail::panel>
