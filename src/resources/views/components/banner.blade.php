...
<div x-data="{{ json_encode(['show' => true, 'style' => $style, 'message' => $message]) }}"
    :class="{ 'bg-[var(--integro-red)]': style == 'success', 'bg-[var(--integro-red)]': style == 'danger', 'bg-amber-600': style == 'warning', 'bg-[var(--integro-gray)]': style != 'success' && style != 'danger' && style != 'warning'}"
            style="display: none;"
            x-show="show && message"
    ...
    <div class="max-w-screen-xl mx-auto py-2 px-3 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between flex-wrap">
            <div class="w-0 flex-1 flex items-center min-w-0">
                <span class="flex p-2 rounded-lg" :class="{ 'bg-black/20': style == 'success', 'bg-black/20': style == 'danger', 'bg-amber-700': style == 'warning' }">
                ...
                <p class="ms-3 font-semibold text-sm text-white truncate" x-text="message"></p>
            </div>

            <div class="shrink-0 sm:ms-3">
                <button
                    type="button"
                    class="-me-1 flex p-2 rounded-md focus:outline-none sm:-me-2 transition hover:bg-black/20 focus:bg-black/20"
                    aria-label="Dismiss"
